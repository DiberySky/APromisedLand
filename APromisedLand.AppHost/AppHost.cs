...
builder
    // .AddIdentity(context)          // Keycloak
    .AddDatabases(context) // Postgres, Redis, RabbitMQ
    // .AddSearch(context)            // Typesense + SearchService
    // .AddSeaweedFs(context)           // SeaweedFS + FileTransService
    .AddOllama(context) // Ollama + embedding + service
    .AddElasticsearch(context);     // Elasticsearch, Kibana, Elasticvue, Elastic-Service
    // .AddQuestionServices(context)  // QuestionService, WeatherApi
    // .AddGateway(context)           // Yarp
    // .AddDevTunnel(context)         // DevTunnel
    // .AddMauiApp(context);          // Maui Blazor
...
var gateway = builder.AddYarp("Yarp")
    .WithConfiguration(yarp =>
    {
        ...
        yarp.AddRoute("/filetrans/{**catch-all}", fileTransService);
    })
