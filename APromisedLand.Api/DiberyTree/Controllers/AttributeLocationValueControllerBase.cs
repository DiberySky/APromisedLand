...
//
// /// <summary>
// /// 定位属性值控制器基类。仿 <see cref="AttributeTableValueControllerBase"/> 分层模式。
...
// [ApiController]
// [Route("[controller]")]
// public abstract class AttributeLocationValueControllerBase(
//     AttributeLocationValueService valueService,
//     ILogger<AttributeLocationValueControllerBase> logger) : ControllerBase
// {
//     // ==================== 定位值 CRUD ====================
//
...
//
//             var (valueId, error, duplicated) = await valueService.AddAsync(nodeId, dto);
//
//             if (duplicated)
...
//
//             var result = await valueService.GetAsync(valueId!);
...
//         try
//         {
//             var list = await valueService.ListByNodeAsync(nodeId);
...
//         try
//         {
//             var dto = await valueService.GetAsync(valueId);
//             if (dto is null)
...
//         try
//         {
//             var error = await valueService.UpdateAsync(valueId, dto);
//             if (!string.IsNullOrEmpty(error))
//                 return BadRequest(ApiResponse<object>.Fail(error));
//
//             var result = await valueService.GetAsync(valueId);
...
//         try
//         {
//             var deleted = await valueService.DeleteAsync(valueId);
//             if (!deleted)
//                 return NotFound(ApiResponse<object>.Fail($"定位值 '{valueId}' 不存在"));
