using APromisedLand.Api.Projects.Elasticsearch.Models;
    ...
    EmbeddingService embeddingService)
{
    ...
    private async Task IndexSampleDataAsync(IEnumerable<DocumentData> documents)
    {
        ...
        foreach (var doc in documents)
        {
            ...
            {
                doc.TitleVector = await embeddingService.GenerateEmbeddingAsync(doc.Title);
            }
            if (doc.ContentVector.Length == 0)
            {
                doc.ContentVector = await embeddingService.GenerateEmbeddingAsync(doc.Content);
            }
