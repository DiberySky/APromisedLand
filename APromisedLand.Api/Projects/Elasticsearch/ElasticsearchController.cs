...
namespace APromisedLand.Api.Projects.Elasticsearch;
    ...
    IEmbeddingService embeddingService) : ControllerBase
{
    ...
    public async Task<IActionResult> Search([FromQuery] string query)
    {
        ...
        try
        {
            ...
            if (!response.IsValidResponse)
                return StatusCode(500, new { error = "Elasticsearch 搜索失败", detail = response.DebugInformation });
        ...
        catch (Exception e)
        {
            ...
            return StatusCode(500, new { error = "Elasticsearch 搜索失败", detail = e.Message });
        }
    }
    ...
    public async Task<IActionResult> SemanticSearch([FromQuery] string query, [FromQuery] int size = 10)
    {
        ...
        try
        {
            var response = await client.SearchAsync<ElasticQuestion>(searchRequest);
            if (!response.IsValidResponse)
                return StatusCode(500, new { error = "语义搜索失败", detail = response.DebugInformation });
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { error = "语义搜索异常", detail = ex.Message });
        }
    }
}